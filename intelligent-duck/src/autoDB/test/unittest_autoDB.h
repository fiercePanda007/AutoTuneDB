...
TEST_CASE("TestBulkLoad") {
  AutoDB<int, int> index;

  AutoDB<int, int>::V values[500];
  for (int i = 0; i < 500; i++) {
...
TEST_CASE("TestConstructors") {
  ...
  AutoDB<int, int> index2(index);  // Copy constructor
  AutoDB<int, int> index3;
  index3 = index;  // Assignment
  AutoDB<int, int> index4(std::begin(values), std::end(values));
...
TEST_CASE("TestIterators") {
  ...
  auto rit = AutoDB<int, int>::ReverseIterator(it);
  num_keys = 0;
  for (; it != index.end(); ++it) {
    num_keys++;
  }
  ...
  typename AutoDB<int, int>::ConstIterator cit = index.find(values[250].first);
  auto crit = AutoDB<int, int>::ConstReverseIterator(cit);
  num_keys = 0;
...
TEST_CASE("TestConst") {
  ...
  const AutoDB<int, int> index(std::begin(values), std::end(values));
...
TEST_CASE("TestLargeFindLastNoGreaterThan") {
  AutoDB<uint64_t, uint64_t> index;
