...
TEST_CASE("TestBinarySearch") {
  AutoDBDataNode<int, int> node;

  AutoDBDataNode<int, int>::V values[100];
  for (int i = 0; i < 100; i++) {
...
TEST_CASE("TestClosestGap") {
  auto node = new AutoDBDataNode<int, int>();
  ...
  node = new AutoDBDataNode<int, int>();
#if AUTODB_DATA_NODE_SEP_ARRAYS
  ...
  for (int i = 300; i < 320; i++) {
    node->key_slots_[i] = AutoDBDataNode<int, int>::kEndSentinel_;
  }
#else
  ...
  for (int i = 300; i < 320; i++) {
    node->data_slots_[i] =
        std::pair<int, int>(AutoDBDataNode<int, int>::kEndSentinel, 0);
  }
#endif
...
TEST_CASE("TestIterator") {
  ...
  AutoDBDataNode<int, int>::const_iterator_type it(&node, 0);
  for (; !it.is_end(); it++) {
...
TEST_CASE("TestIteratorWithDuplicates") {
  ...
  std::vector<int> results;
  AutoDBDataNode<int, int>::const_iterator_type it(&node, node.find_lower(2));
  AutoDBDataNode<int, int>::const_iterator_type end_it(&node, node.find_lower(4));
  for (; it != end_it; it++) {
    results.push_back(it.key());
  }
...
TEST_CASE("TestBulkLoadFromExisting") {
  ...
  AutoDBDataNode<int, int> new_node;
...
TEST_CASE("TestInserts") {
  ...
  AutoDBDataNode<int, int>::V values[200];
  for (int i = 0; i < 200; i++) {
    values[i].first = i;
    values[i].second = i;
  }
...
TEST_CASE("TestBuildIndexWithSample") {
  ...
  AutoDBDataNode<int, int>::V values[num_keys];
  ...
  AutoDBDataNode<int, int>::build_model(values, num_keys, &model, false);
  AutoDBDataNode<int, int>::build_model(values, num_keys, &model_using_sample,
                                      true);
...
TEST_CASE("TestComputeCostWithSample") {
  ...
  LinearModel<int> model;
  AutoDBDataNode<int, int>::build_model(values, num_keys, &model);
  double density = 0.7;
  ...
  AutoDBDataNode<int, int>::compute_expected_cost(
      values, num_keys, density, expected_insert_frac, &model, false, &stats);
  AutoDBDataNode<int, int>::compute_expected_cost(values, num_keys, density,
