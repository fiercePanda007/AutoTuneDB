 ...
 *
 * Core user-facing API of AutoDB:
 * - AutoDB()
...
#include "autoDB_fanout_tree.h"
#include "autoDB_nodes.h"
...
class AutoDB {
  static_assert(std::is_arithmetic<T>::value, "AUTODB key type must be numeric.");
  static_assert(std::is_same<Compare, AutoDBCompare>::value,
                "Must use AutoDBCompare.");

 public:
  // Value type, returned by dereferencing an iterator
  typedef std::pair<T, P> V;

  // AUTODB class aliases
  typedef AutoDB<T, P, Compare, Alloc, allow_duplicates> self_type;
  typedef AutoDBModelNode<T, P, Alloc> model_node_type;
  typedef AutoDBDataNode<T, P, Compare, Alloc, allow_duplicates> data_node_type;
  ...
  AutoDBNode<T, P>* root_node_ = nullptr;
  model_node_type* superroot_ =
      nullptr;  // phantom node that is the root's parent

  /* User-changeable parameters */
  struct Params {
    // When bulk loading, AutoDB can use provided knowledge of the expected
 ...
 public:
  ...
  AutoDB(const Compare& comp, const Alloc& alloc = Alloc())
  ...
  AutoDB(const Alloc& alloc) : allocator_(alloc) {
    // Set up root as empty data node
    auto empty_data_node = new (data_node_allocator().allocate(1))
        data_node_type(key_less_, allocator_);
    empty_data_node->bulk_load(nullptr, 0);
  ...
  explicit AutoDB(InputIterator first, InputIterator last, const Compare& comp,
  ...
  // recommend directly using bulk_load() instead.
  template <class InputIterator>
  explicit AutoDB(InputIterator first, InputIterator last,
                const Alloc& alloc = Alloc())
      : allocator_(alloc) {
    std::vector<V> values;
    for (auto it = first; it != last; ++it) {
  ...
  explicit AutoDB(const self_type& other)
  ...
  AutoDB& operator=(const self_type& other) {
    if (this != &other) {
      for (NodeIterator node_it = NodeIterator(this); !node_it.is_end();
           node_it.next()) {
        delete_node(node_it.current());
 ...
 private:
  ...
  AutoDBNode<T, P>* copy_tree_recursive(const AutoDBNode<T, P>* node) {
    ...
    if (node->is_leaf_) {
      ...
      while (cur < node_copy->num_children_) {
        AutoDBNode<T, P>* child_node = node_copy->children_[cur];
        AutoDBNode<T, P>* child_node_copy = copy_tree_recursive(child_node);
 ...
 public:
  // When bulk loading, AutoDB can use provided knowledge of the expected fraction
...
// node's parent.
 ...
 private:
  ...
  inline void correct_traversal_path(data_node_type* leaf,
    ...
    if (left) {
      ...
      if (start_bucketID == 0) {
        ...
        AutoDBNode<T, P>* cur = parent->children_[correct_bucketID];
        while (!cur->is_leaf_) {
          auto node = static_cast<model_node_type*>(cur);
  ...
  void link_all_data_nodes() {
    ...
    for (NodeIterator node_it = NodeIterator(this); !node_it.is_end();
         node_it.next()) {
      AutoDBNode<T, P>* cur = node_it.current();
      if (cur->is_leaf_) {
 ...
 private:
  ...
  void delete_node(AutoDBNode<T, P>* node) {
    if (node == nullptr) {
      return;
 ...
 private:
  ...
  void create_superroot() {
    ...
    superroot_->children_ =
        new (pointer_allocator().allocate(1)) AutoDBNode<T, P>*[1];
  ...
  // dense keys.
  void bulk_load_node(const V values[], int num_keys, AutoDBNode<T, P>*& node,
                      int total_keys,
    ...
    if (best_fanout_tree_cost < node->cost_ ||
      ...
      model_node->children_ =
          new (pointer_allocator().allocate(fanout)) AutoDBNode<T, P>*[fanout];
 ...
 private:
  ...
  int best_split_propagation(const std::vector<TraversalNode>& traversal_path,
    ...
    for (const TraversalNode& tn : traversal_path) {
      double stop_cost;
      AutoDBNode<T, P>* next = tn.node->children_[tn.bucketID];
  ...
  void expand_root(T key, bool expand_left) {
    ...
    if (static_cast<size_t>(root->num_children_) * expansion_factor <=
      ...
      auto new_children = new (pointer_allocator().allocate(new_num_children))
          AutoDBNode<T, P>*[new_num_children];
      int copy_start;
      if (expand_left) {
      ...
      new_root->children_ = new (pointer_allocator().allocate(expansion_factor))
          AutoDBNode<T, P>*[expansion_factor];
      if (expand_left) {
  ...
  data_node_type* split_upwards(
    ...
    std::vector<AutoDBNode<T, P>*> to_delete;  // nodes that need to be deleted
    ...
    AutoDBNode<T, P>* prev_left_split = left_leaf;
    AutoDBNode<T, P>* prev_right_split = right_leaf;
    ...
    while (traversal_path[path_idx].node->level_ > stop_propagation_level) {
      ...
      AutoDBNode<T, P>* left_half_first_child = cur_node->children_[0];
      AutoDBNode<T, P>* right_half_first_child =
      ...
      // Do the split
      AutoDBNode<T, P>* next_left_split = nullptr;
      AutoDBNode<T, P>* next_right_split = nullptr;
      if (double_left_half) {
        // double left half
        ...
        if (pull_up_right_child) {
          ...
          right_split->children_ =
              new (pointer_allocator().allocate(right_split->num_children_))
                  AutoDBNode<T, P>*[right_split->num_children_];
 ...
 public:
  ...
  // This is just a function required by the STL standard. AUTODB can hold more
  // items.
 ...
 public:
  ...
  bool validate_structure(bool validate_data_nodes = false,
    ...
    bool is_valid = true;
    std::stack<AutoDBNode<T, P>*> node_stack;
    AutoDBNode<T, P>* cur;
 ...
 public:
  ...
  class NodeIterator {
   public:
    ...
    AutoDBNode<T, P>* cur_node_;
    std::stack<AutoDBNode<T, P>*> node_stack_;  // helps with traversal
    ...
    AutoDBNode<T, P>* current() const { return cur_node_; }
