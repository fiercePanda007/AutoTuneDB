    ...
    for (int i = 0; i < level_fanout / 2; i++) {
      if (fanout_tree[level][2 * i].use && fanout_tree[level][2 * i + 1].use) {
        ...
        if (num_node_keys == 0) {
          ...
          best_cost -= kModelSizeWeight * sizeof(AutoDBDataNode<T, P>) *
                       total_keys / num_keys;
          continue;
        }
        ...
        double merging_cost_saving =
            ...
            (kModelSizeWeight * sizeof(AutoDBDataNode<T, P>) * total_keys /
             num_node_keys);
...
double compute_level(const std::pair<T, P> values[], int num_keys,
                     const AutoDBNode<T, P>* node, int total_keys,
  ...
  for (int i = 0; i < fanout; i++) {
    ...
    LinearModel<T> model;
    AutoDBDataNode<T, P>::build_model(values + left_boundary,
                                    right_boundary - left_boundary, &model,
                                    approximate_model_computation);

    DataNodeStats stats;
    double node_cost = AutoDBDataNode<T, P>::compute_expected_cost(
        values + left_boundary, right_boundary - left_boundary,
        AutoDBDataNode<T, P>::kInitDensity_, expected_insert_frac, &model,
  ...
  double traversal_cost =
      ...
      (kModelSizeWeight * fanout *
       (sizeof(AutoDBDataNode<T, P>) + sizeof(void*)) * total_keys / num_keys);
  cost += traversal_cost;
  return cost;
}
...
std::pair<int, double> find_best_fanout_bottom_up(
    const std::pair<T, P> values[], int num_keys, const AutoDBNode<T, P>* node,
  ...
  while (true) {
    ...
    for (FTNode& tree_node : fanout_tree[fanout_tree_level - 1]) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        AutoDBDataNode<T, P>::build_model(values + left, right - left,
                                        &node_models[i],
                                        approximate_model_computation);
        node_costs[i] = AutoDBDataNode<T, P>::compute_expected_cost(
            values + left, right - left, AutoDBDataNode<T, P>::kInitDensity_,
            expected_insert_frac, &node_models[i], approximate_cost_computation,
            &node_stats[i]);
      }
      node_split_cost += sizeof(AutoDBDataNode<T, P>) * kModelSizeWeight *
...
template <class T, class P>
int find_best_fanout_existing_node(const AutoDBModelNode<T, P>* parent,
                                   int bucketID, int total_keys,
                                   std::vector<FTNode>& used_fanout_tree_nodes,
                                   int max_fanout) {
  // Repeatedly add levels to the fanout tree until the overall cost of each
  // level starts to increase
  auto node = static_cast<AutoDBDataNode<T, P>*>(parent->children_[bucketID]);
  ...
  for (int fanout = 1, fanout_tree_level = 0; fanout <= max_fanout;
    ...
    for (int i = 0; i < fanout; i++) {
      ...
      LinearModel<T> model;
      typename AutoDBDataNode<T, P>::const_iterator_type it(node, left_boundary);
      ...
      DataNodeStats stats;
      double node_cost =
          AutoDBDataNode<T, P>::compute_expected_cost_from_existing(
              node, left_boundary, right_boundary,
              AutoDBDataNode<T, P>::kInitDensity_, empirical_insert_frac, &model,
              &stats);
