...
#include "autoDB_base.h"

// Whether we store key and payload arrays separately in data nodes
// By default, we store them separately
#define AUTODB_DATA_NODE_SEP_ARRAYS 1

#if AUTODB_DATA_NODE_SEP_ARRAYS
#define AUTODB_DATA_NODE_KEY_AT(i) key_slots_[i]
#define AUTODB_DATA_NODE_PAYLOAD_AT(i) payload_slots_[i]
#else
#define AUTODB_DATA_NODE_KEY_AT(i) data_slots_[i].first
#define AUTODB_DATA_NODE_PAYLOAD_AT(i) data_slots_[i].second
#endif
...
#define AUTODB_USE_LZCNT 0

namespace autoDB {

// A parent class for both types of AUTODB nodes
...
class AutoDBNode {
 public:
  ...
  AutoDBNode() = default;
  explicit AutoDBNode(short level) : level_(level) {}
  AutoDBNode(short level, bool is_leaf) : is_leaf_(is_leaf), level_(level) {}
  virtual ~AutoDBNode() = default;
...
class AutoDBModelNode : public AutoDBNode<T, P> {
 public:
  typedef AutoDBModelNode<T, P, Alloc> self_type;
  typedef typename Alloc::template rebind<self_type>::other alloc_type;
  typedef typename Alloc::template rebind<AutoDBNode<T, P>*>::other
      pointer_alloc_type;
  ...
  AutoDBNode<T, P>** children_ = nullptr;

  explicit AutoDBModelNode(const Alloc& alloc = Alloc())
      : AutoDBNode<T, P>(0, false), allocator_(alloc) {}

  explicit AutoDBModelNode(short level, const Alloc& alloc = Alloc())
      : AutoDBNode<T, P>(level, false), allocator_(alloc) {}
  ...
  inline AutoDBNode<T, P>* get_child_node(const T& key) {
  ...
  int expand(int log2_expansion_factor) {
    ...
    auto new_children = new (pointer_allocator().allocate(num_new_children))
        AutoDBNode<T, P>*[num_new_children];
    int cur = 0;
    while (cur < num_children_) {
      AutoDBNode<T, P>* cur_child = children_[cur];
  ...
  long long node_size() const override {
    ...
    size += num_children_ * sizeof(AutoDBNode<T, P>*);  // pointers to children
    return size;
  }
  ...
  bool validate_structure(bool verbose = false) const {
    ...
    AutoDBNode<T, P>* cur_child = children_[0];
    int cur_repeats = 1;
    int i;
...
* - Debugging
*/
template <class T, class P, class Compare = AutoDBCompare,
          class Alloc = std::allocator<std::pair<T, P>>,
          bool allow_duplicates = true>
class AutoDBDataNode : public AutoDBNode<T, P> {
 public:
  typedef std::pair<T, P> V;
  typedef AutoDBDataNode<T, P, Compare, Alloc, allow_duplicates> self_type;
  typedef typename Alloc::template rebind<self_type>::other alloc_type;
  ...
  explicit AutoDBDataNode(const Compare& comp = Compare(),
                        const Alloc& alloc = Alloc())
      : AutoDBNode<T, P>(0, true), key_less_(comp), allocator_(alloc) {}

  AutoDBDataNode(short level, int max_data_node_slots,
               const Compare& comp = Compare(), const Alloc& alloc = Alloc())
      : AutoDBNode<T, P>(level, true),
        key_less_(comp),
        allocator_(alloc),
  ...
  AutoDBDataNode(const self_type& other)
      : AutoDBNode<T, P>(other),
  ...
  inline T& get_key(int pos) const { return AUTODB_DATA_NODE_KEY_AT(pos); }

  inline P& get_payload(int pos) const {
    return AUTODB_DATA_NODE_PAYLOAD_AT(pos);
  }
  ...
  void bulk_load(const V values[], int num_keys,
    ...
    if (num_keys == 0) {
      ...
      for (int i = 0; i < data_capacity_; i++) {
        AUTODB_DATA_NODE_KEY_AT(i) = kEndSentinel_;
      }
      return;
    }
    ...
    for (int i = 0; i < num_keys; i++) {
      ...
      if (positions_remaining < keys_remaining) {
        ...
        for (int j = last_position + 1; j < pos; j++) {
          AUTODB_DATA_NODE_KEY_AT(j) = values[i].first;
        }
        for (int j = i; j < num_keys; j++) {
#if AUTODB_DATA_NODE_SEP_ARRAYS
  ...
  void bulk_load_from_existing(
    ...
    for (; it.cur_idx_ < right && !it.is_end(); it++) {
      ...
      if (positions_remaining < keys_remaining) {
        ...
        int pos = data_capacity_ - keys_remaining;
        for (int j = last_position + 1; j < pos; j++) {
          AUTODB_DATA_NODE_KEY_AT(j) = it.key();
        }
    ...
    max_key_ = AUTODB_DATA_NODE_KEY_AT(last_position);
  ...
  int find_key(const T& key) {
    ...
    if (pos < 0 || !key_equal(AUTODB_DATA_NODE_KEY_AT(pos), key)) {
      return -1;
    } else {
      return pos;
    }
  ...
  inline int exponential_search_upper_bound(int m, const K& key) {
    ...
    if (key_greater(AUTODB_DATA_NODE_KEY_AT(m), key)) {
      int size = m;
      while (bound < size &&
             key_greater(AUTODB_DATA_NODE_KEY_AT(m - bound), key)) {
        bound *= 2;
      ...
      while (bound < size &&
             key_lessequal(AUTODB_DATA_NODE_KEY_AT(m + bound), key)) {
        bound *= 2;
        num_exp_search_iterations_++;
      }
      l = m + bound / 2;
  ...
  inline int binary_search_upper_bound(int l, int r, const K& key) const {
    while (l < r) {
      int mid = l + (r - l) / 2;
      if (key_lessequal(AUTODB_DATA_NODE_KEY_AT(mid), key)) {
        l = mid + 1;
      } else {
        r = mid;
      }
  ...
  inline int exponential_search_lower_bound(int m, const K& key) {
    ...
    int bound = 1;
    int l, r;  // will do binary search in range [l, r)
    if (key_greaterequal(AUTODB_DATA_NODE_KEY_AT(m), key)) {
      int size = m;
      while (bound < size &&
             key_greaterequal(AUTODB_DATA_NODE_KEY_AT(m - bound), key)) {
        bound *= 2;
        num_exp_search_iterations_++;
      }
      l = m - std::min<int>(bound, size);
      ...
      int size = data_capacity_ - m;
      while (bound < size && key_less(AUTODB_DATA_NODE_KEY_AT(m + bound), key)) {
        bound *= 2;
        num_exp_search_iterations_++;
      }
      l = m + bound / 2;
  ...
  inline int binary_search_lower_bound(int l, int r, const K& key) const {
    while (l < r) {
      int mid = l + (r - l) / 2;
      if (key_greaterequal(AUTODB_DATA_NODE_KEY_AT(mid), key)) {
        r = mid;
      } else {
        l = mid + 1;
      }
  ...
  std::pair<int, int> insert(const T& key, const P& payload) {
    ...
    if (!allow_duplicates && upper_bound_pos > 0 &&
        key_equal(AUTODB_DATA_NODE_KEY_AT(upper_bound_pos - 1), key)) {
  ...
  void insert_element_at(const T& key, P payload, int pos) {
    ...
    while (pos >= 0 && !check_exists(pos)) {
      AUTODB_DATA_NODE_KEY_AT(pos) = key;
      pos--;
    }
  }
  ...
  int erase_one(const T& key) {
    ...
    if (pos == data_capacity_ || !key_equal(AUTODB_DATA_NODE_KEY_AT(pos), key))
      return 0;
  ...
  void erase_one_at(int pos) {
    ...
    if (pos == data_capacity_ - 1) {
      ...
      next_key = AUTODB_DATA_NODE_KEY_AT(pos + 1);
    }
    AUTODB_DATA_NODE_KEY_AT(pos) = next_key;
    unset_bit(pos);
    pos--;
  ...
  int erase(const T& key) {
    ...
    if (pos == 0 || !key_equal(AUTODB_DATA_NODE_KEY_AT(pos - 1), key)) return 0;
    ...
    if (pos == data_capacity_) {
      ...
      next_key = AUTODB_DATA_NODE_KEY_AT(pos);
    }
    pos--;
    while (pos >= 0 && key_equal(AUTODB_DATA_NODE_KEY_AT(pos), key)) {
      AUTODB_DATA_NODE_KEY_AT(pos) = next_key;
  ...
  int erase_range(T start_key, T end_key, bool end_key_inclusive = false) {
    ...
    pos--;
    while (pos >= 0 &&
           key_greaterequal(AUTODB_DATA_NODE_KEY_AT(pos), start_key)) {
      AUTODB_DATA_NODE_KEY_AT(pos) = next_key;
      num_erased += check_exists(pos);
      unset_bit(pos);
      pos--;
  ...
  bool validate_structure(bool verbose = false) const {
    ...
    for (int i = 0; i < data_capacity_ - 1; i++) {
      if (key_greater(AUTODB_DATA_NODE_KEY_AT(i), AUTODB_DATA_NODE_KEY_AT(i + 1))) {
        if (verbose) {
          std::cout << "Keys should be in non-increasing order" << std::endl;
        }
        return false;
      } else if (key_less(AUTODB_DATA_NODE_KEY_AT(i),
                          AUTODB_DATA_NODE_KEY_AT(i + 1)) &&
                 !check_exists(i)) {
        if (verbose) {
    ...
    if (AUTODB_DATA_NODE_KEY_AT(data_capacity_ - 1) == kEndSentinel_ &&
  ...
  bool key_exists(const T& key, bool validate_bitmap) const {
    for (int i = 0; i < data_capacity_ - 1; i++) {
      if (key_equal(AUTODB_DATA_NODE_KEY_AT(i), key) &&
  ...
  std::string to_string() const {
    ...
    for (int i = 0; i < data_capacity_; i++) {
      str += (std::to_string(AUTODB_DATA_NODE_KEY_AT(i)) + " ");
    }
    return str;
  }
};
